...
description: This chapter explains the concept of a Teleport Node and Teleport manages SSH.
...
- You connect nodes to the proxy using outbound persistent tunnels, for your IoT lab or remote infrastructure.
...
A node candidate becomes a Teleport Node it joins a cluster - authenticates itself to receive cluster certificate.
...
Clients can access the list of all Nodes in their cluster via the [Auth Server API](authentication.mdx#auth-api) or CLI.
