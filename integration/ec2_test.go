...
func newNodeConfig(t *testing.T, authAddr utils.NetAddr, tokenName string, joinMethod types.JoinMethod) *service.Config {
	...
	config.SSH.Addr.Addr = helpers.NewListener(t, service.ListenerNodeSSH, &config.FileDescriptors) // net.JoinHostPort(Host, helpers.NewPortStr())
...
func newProxyConfig(t *testing.T, authAddr utils.NetAddr, tokenName string, joinMethod types.JoinMethod) *service.Config {
	...
	proxyAddr := helpers.NewListener(t, service.ListenerProxyWeb, &config.FileDescriptors) // net.JoinHostPort(Host, helpers.NewPortStr())
...
func newAuthConfig(t *testing.T, clock clockwork.Clock) *service.Config {
	...
	config := service.MakeDefaultConfig()
	config.DataDir = t.TempDir()
	config.Auth.SSHAddr.Addr = helpers.NewListener(t, service.ListenerAuthSSH, &config.FileDescriptors)
...
func TestEC2Labels(t *testing.T) {
	...
	tconf.Auth.SSHAddr.Addr = helpers.NewListener(t, service.ListenerAuthSSH, &tconf.FileDescriptors)
